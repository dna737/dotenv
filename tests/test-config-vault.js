...
const dotenvKey =
  "dotenv://:key_ddcaa26504cd70a6fef9801901c3981538563a1767c297cb8416e8a38c62fe00@dotenv.org/vault/.env.vault?environment=development";

let envStub;
let logStub;

t.beforeEach(() => {
  import.meta.env.DOTENV_KEY = "";
  envStub = sinon.stub(import.meta.env, "DOTENV_KEY").value(dotenvKey);
});
...
t.test(
  "logs when testPath calls to .env.vault directly (interpret what the user meant)",
  (ct) => {
    ct.plan(1);
...
t.test("throws not found if .env.vault is empty", (ct) => {
  ...
  try {
    ...
    ct.equal(
      e.message,
      "NOT_FOUND_DOTENV_ENVIRONMENT: Cannot locate environment DOTENV_VAULT_DEVELOPMENT in your .env.vault file."
    );
  }
...
t.test("throws missing data when somehow parsed badly", (ct) => {
  ...
  const configDotenvStub = sinon
    .stub(dotenv, "configDotenv")
    .returns({ parsed: undefined });
  ...
  try {
    ...
    ct.equal(
      e.message,
      "MISSING_DATA: Cannot parse tests/.env.vault for an unknown reason"
    );
  }
...
t.test("throws error when invalid formed DOTENV_KEY", (ct) => {
  envStub.restore();
  envStub = sinon
    .stub(import.meta.env, "DOTENV_KEY")
    .value("invalid-format-non-uri-format");
...
t.test(
  "throws error when invalid formed DOTENV_KEY that otherwise is not caught",
  (ct) => {
    ct.plan(1);
...
t.test("throws error when DOTENV_KEY missing password", (ct) => {
  ...
  envStub = sinon
    .stub(import.meta.env, "DOTENV_KEY")
    .value(
      "dotenv://username@dotenv.org/vault/.env.vault?environment=development"
    );
...
t.test("throws error when DOTENV_KEY missing environment", (ct) => {
  ...
  envStub = sinon
    .stub(import.meta.env, "DOTENV_KEY")
    .value(
      "dotenv://:key_ddcaa26504cd70a6fef9801901c3981538563a1767c297cb8416e8a38c62fe00@dotenv.org/vault/.env.vault"
    );
...
t.test("when DOTENV_KEY is empty string falls back to .env file", (ct) => {
  envStub.restore();
  envStub = sinon.stub(import.meta.env, "DOTENV_KEY").value("");
...
t.test(
  "does not write over keys already in import.meta.env by default",
  (ct) => {
    ct.plan(2);

    const existing = "bar";
    import.meta.env.ALPHA = existing;

    const result = dotenv.config({ path: testPath });

    ct.equal(result.parsed.ALPHA, "zeta");
    ct.equal(import.meta.env.ALPHA, "bar");
  }
);

t.test(
  "does write over keys already in import.meta.env if override turned on",
  (ct) => {
    ct.plan(2);
    ...
    ct.equal(result.parsed.ALPHA, "zeta");
    ct.equal(import.meta.env.ALPHA, "zeta");
  }
);

t.test(
  "when DOTENV_KEY is passed as an option it successfully decrypts and injects",
  (ct) => {
    envStub.restore();
    envStub = sinon.stub(import.meta.env, "DOTENV_KEY").value("");
...
t.test("can write to a different object rather than import.meta.env", (ct) => {
  ct.plan(3);

  import.meta.env.ALPHA = "other"; // reset import.meta.env
  ...
  ct.equal(result.parsed.ALPHA, "zeta");
  ct.equal(import.meta.env.ALPHA, "other");
...
t.test("raises an DECRYPTION_FAILED if key fails to decrypt payload", (ct) => {
  ...
  envStub = sinon
    .stub(import.meta.env, "DOTENV_KEY")
    .value(
      "dotenv://:key_2c4d267b8c3865f921311612e69273666cc76c008acb577d3e22bc3046fba386@dotenv.org/vault/.env.vault?environment=development"
    );
...
t.test(
  "raises an DECRYPTION_FAILED if both (comma separated) keys fail to decrypt",
  (ct) => {
    envStub.restore();
    envStub = sinon
      .stub(import.meta.env, "DOTENV_KEY")
      .value(
        "dotenv://:key_2c4d267b8c3865f921311612e69273666cc76c008acb577d3e22bc3046fba386@dotenv.org/vault/.env.vault?environment=development,dotenv://:key_c04959b64473e43dd60c56a536ef8481388528b16759736d89515c25eec69247@dotenv.org/vault/.env.vault?environment=development"
      );
