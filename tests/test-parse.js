...
const parsed = dotenv.parse(
  fs.readFileSync("tests/.env", { encoding: "utf8" })
);
...
t.equal(
  parsed.DOUBLE_QUOTES_WITH_NO_SPACE_BRACKET,
  "{ port: $MONGOLAB_PORT}",
...
t.equal(
  parsed.RETAIN_INNER_QUOTES_AS_BACKTICKS,
  '{"foo": "bar\'s"}',
  "retains inner quotes"
);
