...
t.test("reads path with encoding, parsing output to import.meta.env", (ct) => {
  ct.plan(2);
...
t.test("does not write over keys already in import.meta.env", (ct) => {
  ct.plan(2);
...
t.test(
  ...
  (ct) => {
    ...
    ct.equal(env.parsed && env.parsed.test, mockParseResponse.test);
    ct.equal(import.meta.env.test, "foo");
  }
);

t.test(
  "does not write over keys already in import.meta.env if the key has a falsy value",
  (ct) => {
    ct.plan(2);
    ...
    ct.equal(env.parsed && env.parsed.test, mockParseResponse.test);
    // NB: import.meta.env.test becomes undefined on Windows
    ct.notOk(import.meta.env.test);
  }
);

t.test(
  "does write over keys already in import.meta.env if the key has a falsy value but override is set to true",
  (ct) => {
    ct.plan(2);
...
t.test("can write to a different object rather than import.meta.env", (ct) => {
  ...
  import.meta.env.test = "other"; // reset import.meta.env

  const myObject = {};
  const env = dotenv.config({ processEnv: myObject });

  ct.equal(env.parsed && env.parsed.test, mockParseResponse.test);
  console.log("logging", import.meta.env.test);
  ct.equal(import.meta.env.test, "other");
...
t.test(
  "logs any errors thrown from reading file or parsing when in debug mode",
  (ct) => {
    ct.plan(2);
